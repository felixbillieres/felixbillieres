---
description: https://github.com/sec-fortress/CRTP-Notes/tree/main
---

# 🍂 Labs Methodology

{% hint style="danger" %}
All of the content is part of the github above, I take it and add things i think are usefull
{% endhint %}
...
Get-DomainUser | clear -ExpandProperty samaccountname
```
...
* Navigate to the bloodhound collector directory on the GUI and drag and drop the zip file to Bloodhound\_old UI
...
* We can then use `BetterSafetyKatz.exe` to create a Golden ticket using the **aes256\_hmac** key from last output
...
* [ ] _**Create Invoke-PowerShellTcpEx.ps1:**_
...
* Check if the tickets are present, _**Desired Output -:**_
...
> \[!note] Make sure you don't make the mistake of copying the trust key (rc4\_hmac\_nt), of `MONEYCORP.LOCAL` instead of `EUROCORP.LOCAL` as used in the below command "`/rc4:`", scrolling down a little bit on the command output will help
...
> \[!note] Copy all the text between `-----BEGIN RSA PRIVATE KEY-----` and `-----END CERTIFICATE-----` and save it to `esc1.pem`.
...
> \[!note] Difference in `/altname`, Then go ahead and save the certificate and save it as `esc1-EA.pem`.
